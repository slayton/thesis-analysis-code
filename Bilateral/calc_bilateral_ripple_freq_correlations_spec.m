...
idx = 1;
for i = 1:nAnimal
    n = numel( ripples(i).peakFr{1} );
    shuffdIdx = randsample(n, n);
    
    frBase( idx:idx + n - 1 ) = ripples(i).peakFr{1};
    frCont( idx:idx + n - 1) = ripples(i).peakFr{3};
    idx = idx + n;
end
...
for sCount = 1:nShuffle
    ...
    for i = 1:nAnimal
        n = numel( ripples(i).peakFr{1} );
        shuffdIdx = randsample(n, n);
        frShuf1(idx:idx + n - 1) = ripples(i).peakFr{1}(shuffdIdx, 1);   
        idx = idx + n;
    end
