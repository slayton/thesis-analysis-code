...
meanMuRate = nanmean( mu.hpc(isStopped) );
stdMuRate = nanstd( mu.hpc(isStopped) );
...
high_seg = logical2seg(mu.ts, mu.hpc >= highThreshold);
low_seg =  logical2seg(mu.ts, mu.hpc >= lowThreshold);
